		...
		$.ajax({
			...
			success: function(d) {
				...
				*/
				...
				query(' SELECT ?a ?tp ?lb ?id ?st ?en ?nm ?ml ?dt ?val ?v1 ?v2 ?v3 \
					WHERE { ?a a oa:Annotation ; oa:hasTarget ?t ;  oa:annotatedBy ?aut ; oa:annotatedAt ?dt ; oa:hasBody ?b .\
					?t a oa:SpecificResource. ?t oa:hasSource <'+prefixUri['ao'] + title + '.html'+'>. ?t oa:hasSelector ?s .\
					?s a oa:FragmentSelector ; oa:end ?en ; oa:start ?st ; rdf:value ?id . ?aut foaf:name ?nm ; schema:email ?ml .\
					?b rdf:object ?val .\
					OPTIONAL { ?a rdfs:label ?lb . } \
